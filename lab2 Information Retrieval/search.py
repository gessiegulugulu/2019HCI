...
################################################################################################################################
#import rest_server
#from rest_server import tagtxt_name
import random
...
tag_list=['animals','people','food','structures','transport']
def get_top_k_similar(image_data, pred, pred_final, k):
        ...
        if not gfile.Exists('static/people'):
            os.mkdir('static/people')
        if not gfile.Exists('static/food'):
            os.mkdir('static/food')
        if not gfile.Exists('static/structures'):
            os.mkdir('static/structures')
        if not gfile.Exists('static/transport'):
            os.mkdir('static/transport')
    ...
    # cosine calculates the cosine distance, not similiarity. Hence no need to reverse list
        ...
        for i, neighbor in enumerate(top_k_ind):
            ...
            for tagName in tag_list:
                tagpath='database/tags/'+tagName+'.txt'
